...
export const sendMail=async (req: Request, res: Response) => {
  ...
  var transportersnd = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user:"mnbro123321@gmail.com" ,
      pass: "hbqa kzdq keop rxbl",
  ...
  transportersnd.verify(function (error, success) {
    if (error) {
      console.log(error);
    } else {
      console.log("Server is ready to take our messages");
  ...
  try {
      ...
      var mailOptions = {
        ...
        to: Email,
        subject: "You Are Added as Location Admin in Homly",
        html: "Your reset password is sadnjd123 make sure to chnage it when you logged in"
      };

    

    transportersnd.sendMail(mailOptions, function (error: any, info) {
      if (error) {
        console.log(error);
      } else {
        console.log("Email sent: " + info.response);
